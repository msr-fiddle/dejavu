...
template<typename T>
void handleOptArgWithOffset(TensorMap* input_tensors, const std::string& arg_name, T* d_ptr, T default_value, size_t offset, size_t size)
{
    if (input_tensors->isExist(arg_name)) {
        FT_CHECK(input_tensors->at(arg_name).size() == size);
        cudaH2Dcpy(d_ptr, input_tensors->at(arg_name).getPtr<const T>() + offset, size);
    }
    else {
        deviceFill(d_ptr, size, default_value);
    }
...
void setSeqLimitLenWithOffset(uint32_t* seq_len_d, Tensor seq_len, int limit_len_offset, int batch_size, int ptr_offset);
