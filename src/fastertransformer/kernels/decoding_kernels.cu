...
__global__ void single_token_transfer(void*  src_start,
                                      void*  dst_start,
                                      size_t size,
                                      size_t stride,
                                      size_t times,
                                      ...
                                      int    bytes)
{
    ...
    char*  src    = (char*)src_start + idx * row_size + (step - 1) * size * bytes;
    char*  dst    = (char*)dst_start + offset;
    if (offset <= token_cache_size - size * bytes) {
        int total_bytes = size * bytes;
        assert(total_bytes % 8 == 0);
        int long_times = total_bytes / 8;
        for (int i = 0; i < long_times; i++) {
            *((unsigned long long*)dst + i) = *((unsigned long long*)src + i);
        }
    }
}

__global__ void transfer_kernel_prompt(
    void* src_start, void* dst_start, size_t src_stride, size_t dst_stride, size_t per_token_size, int bytes)
{

    // each of the threads is responsible for 'per_token_size' bytes

    size_t block_idx = (size_t)(blockIdx.x);

    size_t src_offset = block_idx * src_stride + per_token_size * bytes * (size_t)(threadIdx.x);
    size_t dst_offset = block_idx * dst_stride + per_token_size * bytes * (size_t)(threadIdx.x);
    char*  src        = (char*)src_start + src_offset;
    char*  dst        = (char*)dst_start + dst_offset;

    int total_bytes = per_token_size * bytes;
    assert(total_bytes % 8 == 0);

    // to accelerate transfering
    int long_times = total_bytes / 8;
    for (int i = 0; i < long_times; i++) {
        *((unsigned long long*)dst + i) = *((unsigned long long*)src + i);
    }
...
__global__ void multi_token_transfer(void*  src_start,
                                     void*  dst_start,
                                     ...
                                     int    bytes)
{
    ...
    int    factor      = times / num_threads_per_block;
    if (times % num_threads_per_block > 0)
        factor += 1;

    size_t my_step     = blockIdx.x / factor;
    size_t step_offset = threadIdx.x + (blockIdx.x % factor) * blockDim.x;

    if (step_offset >= times)
        return;

    // printf("factor is %d, my_step is %lu, step_offset is %lu\n", factor, my_step, step_offset);

    size_t dst_offset = step_offset * row_size + (step + my_step - 1) * size * bytes;
    size_t src_offset = my_step * token_cache_size + step_offset * stride;

    // printf("my_step %lu, dst_offset: %lu, src_offset: %lu\n", my_step, dst_offset, src_offset);
...
void invoke_single_token_transfer(void*        src_start,
                                  void*        dst_start,
                                  size_t       size,
                                  size_t       stride,
                                  size_t       times,
                                  ...
                                  cudaStream_t stream)
{
    ...
    int num_threads = 96;
    int num_blocks  = times / num_threads;

    if (times % num_threads > 0)
        num_blocks += 1;

    single_token_transfer<<<num_blocks, num_threads, 0, stream>>>(
        src_start, dst_start, size, stride, times, step, row_size, token_cache_size, bytes);
}

void invoke_transfer_kernel_prompt(void*        src_start,
                                   void*        dst_start,
                                   size_t       times,
                                   ...
                                   cudaStream_t stream)
{
    ...
    int num_threads = num_transfers;
    int num_blocks  = times;

    transfer_kernel_prompt<<<num_blocks, num_threads, 0, stream>>>(
        src_start, dst_start, src_offset, dst_offset, per_token_size, bytes);
}

void invoke_multi_token_transfer(void*        src_start,
                                 void*        dst_start,
                                 size_t       size,
                                 size_t       stride,
                                 int          times,
                                 ...
                                 cudaStream_t stream)
{
    ...
    int num_threads = 96;
    int num_blocks  = num_steps * times / num_threads;

    if ((num_steps * times) % num_threads > 0)
        num_blocks += 1;

    multi_token_transfer<<<num_blocks, num_threads, 0, stream>>>(
        src_start, dst_start, num_threads, size, stride, times, start_step, row_size, token_cache_size, bytes);
}
